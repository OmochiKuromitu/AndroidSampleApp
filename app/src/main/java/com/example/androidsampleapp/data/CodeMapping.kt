...
// 機器と API がやり取りに使う文字列と、アプリの型との対応。
//
// これは「通信の言葉 → アプリの言葉」の変換なので data 層に置く。domain に置くと、
// domain がサーバや機器の文字列を知ることになり、形式が変わったときに domain まで直す羽目になる。
// 複数のリポジトリ（DeviceRepositoryImpl と NoticeRepositoryImpl）が同じ対応を使うので、
// どちらかの private にはせず、ここに 1 つだけ持つ。
//
// 知らない文字列が来たときにどう読むか（既定値）も、読み替えの判断としてここで決める。

/** 機器から届いたモードの文字列を読む。知らない値なら冷房として扱う。 */
...
/** 機器へ送るときのモードの文字列。[airconModeOf] の逆向き。 */
